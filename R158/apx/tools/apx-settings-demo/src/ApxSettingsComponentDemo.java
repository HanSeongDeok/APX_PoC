...
public final class ApxSettingsComponentDemo {
    ...
    private static String doc(String role, String pathFromSrc, String exampleJavaPath,
        ...
        sb.append("\n\n※ Composite parent = SWT Composite(부모 컨테이너)\n예시 (기본): \n").append(defaultEx);
    ...
    private Item cameraSelectItem() {
        return new Item(
                ...
                doc("  웹캠 선택 콤보 + 새로고침 버튼.\n"
                                ...
                                + "  ※ ApxSettings: MicSelectBar가 저장해 둔 값.\n"
                                + "  ※ 채널을 반드시 지정할 것. 인자 하나짜리는 클러스터로 붙는다.\n"
                                + "  ※ 현재는 2채널(CLUSTER / GEAR)까지만 지원.\n"
                                + "  ※ CameraService가 두 개로 고정이고 3번째 채널은 지원하지 않습니다.",
                        SRC_UI + "settings/vision/CameraSelectBar.java",
                        EX_COMP + "CameraSelectBarExample.java",
                        "CameraSelectBar cam = new CameraSelectBar(parent, VisionChannel.CLUSTER);\n"
                                + "CameraCanvas canvas = new CameraCanvas(parent);\n"
                                + "canvas.setPlaceholder(\"웹캠을 선택하세요\");\n"
                                + "cam.setCanvas(canvas);  // 프레임 => canvas\n"
                                + "cam.refreshCameras();   // 장치 목록 로드",
                        "// 문구 커스텀\n"
                                + "CameraSelectBar.Cfg cfg = new CameraSelectBar.Cfg();\n"
                                + "cfg.refreshText    = \"카메라 목록 다시 읽기\";\n"
                                + "cfg.comboTooltip   = \"전방/후방 카메라를 각각 고르세요\";\n"
                                + "cfg.refreshTooltip = \"USB를 다시 꽂았을 때 누르세요\";\n"
                                + "CameraSelectBar cam =\n"
                                + "new CameraSelectBar(parent, VisionChannel.GEAR, cfg);\n"
                                + "\n"
                                + "// 2채널 - 바 2개 + 화면 2개 (채널마다 한 쌍)\n"
                                + "// 채널을 안 주면 둘 다 클러스터로 붙어 같은 카메라를 잡는다.\n"
                                + "for (VisionChannel ch : new VisionChannel[] {\n"
                                + "VisionChannel.CLUSTER, VisionChannel.GEAR }) {\n"
                                + "    Group g = new Group(parent, SWT.NONE);\n"
                                + "    g.setText(ch.label + \" 웹캠\");\n"
                                + "    g.setLayout(new GridLayout(1, false));\n"
                                + "    g.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));\n"
                                + "\n"
                                + "    CameraSelectBar bar = new CameraSelectBar(g, ch);\n"
                                + "    CameraCanvas cv = new CameraCanvas(g);\n"
                                + "    cv.setPlaceholder(ch.label + \" 웹캠을 선택하세요\");\n"
                                + "    GridData cvGd = new GridData(SWT.FILL, SWT.FILL, true, true);\n"
                                + "    cvGd.heightHint = 160;\n"
                                + "    cv.setLayoutData(cvGd);\n"
                                + "\n"
                                + "    bar.setCanvas(cv);      // 이 채널 프레임 => 이 Canvas\n"
                                + "    bar.refreshCameras();   // 비어 있는 장치부터 자동 선택\n"
                                + "}\n"
                                + "// 3채널 이상은 불가.\n"),
                new DemoEntry() {
                    public void create(Composite parent) {
                        // 2채널 - 채널마다 바 + 화면 한 쌍. 3채널 이상은 불가.
                        VisionChannel[] chs = {
                                VisionChannel.CLUSTER, VisionChannel.GEAR };
                        for (int i = 0; i < chs.length; i++) {
                            VisionChannel ch = chs[i];
                            Group g = new Group(parent, SWT.NONE);
                            g.setText(ch.label + " 웹캠");
                            g.setLayout(new GridLayout(1, false));
                            g.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
    ...
    private Item roiNccItem() {
        return new Item(
                ...
                doc("  화면 위에서 마우스로 사각형(ROI=비교할 영역)을 그리고,\n"
                                ...
                                + "  ※ ApxSettings: 그린 사각형 좌표(ROI) 저장. 합격은 NCC 점수(임계치) 이상.",
    ...
    private Item micSelectItem() {
        return new Item(
                ...
                doc("  마이크 선택 콤보 + 새로고침(선택만 담당).\n"
                                ...
                                + "  ※ 측정 바는 이 이름을 읽어 실제 마이크를 엽니다. (AudioCapture.findInputDevice).\n"
                                + "  ※ 측정 바는 설정값(micName)으로만 연결.",
                        SRC_UI + "settings/audio/MicSelectBar.java",
                        EX_COMP + "MicSelectBarExample.java",
                        "MicSelectBar micBar = new MicSelectBar(parent);\n"
                                + "micBar.refreshMics();",
                        "// 문구 커스텀\n"
                                + "MicSelectBar.Cfg cfg = new MicSelectBar.Cfg();\n"
                                + "cfg.refreshText    = \"마이크 목록 다시 읽기\";\n"
                                + "cfg.comboTooltip   = \"경고음을 받을 마이크를 고르세요\";\n"
                                + "cfg.refreshTooltip = \"USB를 다시 꽂았을 때 누르세요\";\n"
                                + "MicSelectBar micBar = new MicSelectBar(parent, cfg);\n"
                                + "micBar.refreshMics();\n"),
                new DemoEntry() {
                    public void create(Composite parent) {
    ...
    private Item micTestItem() {
        return new Item(
                ...
                doc("  입력 레벨 막대 + '마이크 테스트' 시작/정지 버튼.\n"
                                ...
                                + "  ※ ApxSettings: micName을 읽어 마이크를 엽니다(MicSelectBar가 저장해 둔 값).\n"
                                + "  ※ 문구 3개(레벨 라벨 / 시작 / 정지)를 Cfg로 받는다.\n"
                                + "     정지 문구는 테스트 ON 일 때 버튼에 바뀌어 들어간다.",
                        SRC_UI + "settings/audio/MicTestBar.java",
                        EX_COMP + "MicTestBarExample.java",
                        "new MicTestBar(parent); // 장치는 ApxSettings.micName으로 해석",
                        "MicTestBar.Cfg cfg = new MicTestBar.Cfg();\n"
                                + "cfg.levelText   = \"마이크 입력\";\n"
                                + "cfg.testText    = \"소리 확인 시작\";\n"
                                + "cfg.testingText = \"소리 확인 정지\";\n"
                                + "new MicTestBar(parent, cfg);"),
                new DemoEntry() {
                    public void create(Composite parent) {
        ...
        return new Item(
                ...
                doc("  비교 기준이 될 '기대 경고음(.wav)' 파일 선택 UI.\n"
                        ...
                        "ExpectedWavBar.Cfg cfg = new ExpectedWavBar.Cfg();\n"
                                + "cfg.titleText       = \"기대 경고음 파일 (.wav)\";  // 위쪽 라벨\n"
                                + "cfg.pickText        = \"찾아보기...\";              // 파일 선택 버튼\n"
                                + "cfg.placeholderText = \"경고음 .wav를 선택하세요\";  // 미선택 시 표시\n"
                                + "cfg.dialogText      = \"경고음 파일을 고르세요\";    // 대화상자 제목\n"
                                + "cfg.filterName      = \"경고음 (*.wav)\";           // 파일 형식 이름\n"
    ...
    private Item audioMeasureItem() {
        return new Item(
                "AudioMeasureBar",
                "AudioMeasureBar",
                doc("  측정 ON 시 마이크를 열어 기대음과 실시간 비교 + setScope로 연결한 그래프에 파형 표시.\n"
    ...
    private Item rearGridSizeItem() {
        return new Item(
                "RearGridSizeBar",
                "RearGridSizeBar",
                doc("  후방 격자 크기 UI만 담당(프리셋 라디오+콤보 / 커스텀 스피너).  모드 전환 시 편집 컨트롤은 같은 자리에 하나만 표시.\n"
